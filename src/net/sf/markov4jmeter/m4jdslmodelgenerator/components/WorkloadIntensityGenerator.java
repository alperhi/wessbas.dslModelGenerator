...
public class WorkloadIntensityGenerator {
	...
	private final static String PKEY_WORKLOAD_INTENSITY__FORMULA = "wl.type.value";
	...
	/**
	 * Error message for the case that a given workload intensity formula does
	 * not denote a constant workload intensity.
	 */
	private final static String ERROR_NO_CONSTANT_WORKLOAD_INTENSITY = "formula \"%s\" does not denote a constant workload intensity";

	/**
	 * Error message for the case that a specified workload intensity type is
	 * unknown.
	 */
	private final static String ERROR_UNKNOWN_WORKLOAD_INTENSITY_TYPE = "unknown workload intensity type \"%s\"";

	/**
	 * Error message for the case that no workload intensity formula is defined.
	 */
	private final static String ERROR_WORKLOAD_INTENSITY_FORMULA_UNDEFINED = "workload intensity formula undefined";

	/**
	 * Error message for the case that no workload intensity type is defined.
	 */
	private final static String ERROR_WORKLOAD_INTENSITY_TYPE_UNDEFINED = "workload intensity type undefined";
	 ...
	 *             if the workload intensity information provided by the
	 *             properties is insufficient or invalid.
	 */
	public WorkloadIntensity generateWorkloadIntensity(
		...
		final String type = properties
				.getProperty(WorkloadIntensityGenerator.PKEY_WORKLOAD_INTENSITY__TYPE);

		final String formula = properties
				.getProperty(WorkloadIntensityGenerator.PKEY_WORKLOAD_INTENSITY__FORMULA);

		if (type == null) {

			throw new GeneratorException(
					WorkloadIntensityGenerator.ERROR_WORKLOAD_INTENSITY_TYPE_UNDEFINED);
		}

		if (formula == null) {

			throw new GeneratorException(
					WorkloadIntensityGenerator.ERROR_WORKLOAD_INTENSITY_FORMULA_UNDEFINED);
		}
		...
		default:
			...
			final String message = String
					.format(WorkloadIntensityGenerator.ERROR_UNKNOWN_WORKLOAD_INTENSITY_TYPE,
							type);
	 ...
	 * 
	 * @param formula
	 *            formula which indicates a constant number of sessions.
	 * 
	 * @return the newly created M4J-DSL model component.
	 * 
	 * @throws GeneratorException
	 *             if the formula does not denote a number.
	 */
	private ConstantWorkloadIntensity generateConstantWorkloadIntensity(
			final String formula) throws GeneratorException {

		final ConstantWorkloadIntensity constantWorkloadIntensity = this.m4jdslFactory
				.createConstantWorkloadIntensity();
	...
	private int getConstantNumberOfSessions(final String formula)
		...
		try {
			...
			final String message = String
					.format(WorkloadIntensityGenerator.ERROR_NO_CONSTANT_WORKLOAD_INTENSITY,
							formula);
